...
# Function to load a model from the models folder, cached to avoid reloading
@st.cache_resource
...
# Dictionary mapping crop names to model file paths and class names (adjust as needed)
MODELS = {
    "Potato": {
        "path": os.path.join("models", "potato_model_epoch_39.keras"),
        "classes": ["Early Blight", "Late Blight", "Healthy", "random"],
    },
    "Corn": {
        "path": os.path.join("models", "corn_model.keras"),
        "classes": ["Gray Leaf Spot", "Common Rust", "Northern Leaf Blight", "Healthy"],
    },
    "Apple": {
        "path": os.path.join("models", "apple_model.keras"),
...
# Load the appropriate model and class labels
model_info = MODELS[crop]
model_path = model_info["path"]
class_names = model_info["classes"]

# Title and limitation notice
